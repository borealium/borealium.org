satni = Sátni.org
satni-description = Yrkorð og orðabøkur fyri sámisk mál
satni-more-info =
    Sátni.org hevur yrkorð frá Giellagáldu og ymsar orðabøkur fyri sámisk mál.
    Tú kanst leita í øllum í senn, og tú kanst síla úrslitini eftir máli og orðabók.
satni-links-0 = Sátni.org
divvun-keyboard = Divvun-knappaborð
divvun-keyboard-description = Divvun-knappaborð til iOS og Android
divvun-keyboard-more-info = Divvun-knappaborð hava knappaborð til tey flestu sámisku málini umframt til nøkur onnur minniulutamál og mál hjá upprunafólki.
divvun-keyboard-links-0 = Støðug útgáva
divvun-keyboard-links-1 = Støðug útgáva
divvun-keyboard-links-2 = Í menning
divvun-keyboard-links-3 = Í menning
divvun-manager = Divvun Manager
divvun-manager-description = Divvun Manager er eitt forrit, sum leggur máltøknilig amboð inn á tína Windows- ella Mac-teldu.
divvun-manager-more-info =
    Á Divvun Manager velur tú eitt mál, og síðani leggur tú tey amboðini, tú vilt hava, inn á telduna.
    Eftir tað kannar Divvun Manager regluliga, um nakrar dagføringar eru komnar og leggur tær sjálvvirkandi
    inn á telduna.
divvun-manager-links-0 = Divvun Manager
divvun-manager-links-1 = Divvun Manager
divvunspell-libreoffice = DivvunSpell til LibreOffice
divvunspell-libreoffice-description = Eitt ískoytisforrit til LibreOffice, sum gevur atgongd til rættstavarar hjá Divvun.
divvunspell-libreoffice-more-info =
    Á Windows brúkar tú Divvun Manager til at leggja hetta ískoytisforritið inn á telduna. Tað verður lagt inn sjálvvirkandi, um tú hevur LibreOffice á telduni.
    
    Legg tað á tína macOS-teldu
    ---------------------
    
    - Legg allar viðkomandi rættstavarar inn við Divvun Manager
    - Tak .oxt-fíluna niðanfyri niður á telduna
    - Tak .oxt-fíluna inn í ískoytisforritini (plugins) í LibreOffice
    - Endurtendra LibreOffice
    
    Nú kanst tú brúka rættstavaran á málunum, sum tú longu hevur lagt inn.
divvunspell-libreoffice-links-0 = .oxt-fíla
gramdivvun = GramDivvun
gramdivvun-description = Mállærurættarar hjá Divvun
gramdivvun-more-info =
    GramDivvun er eitt ískoytisforrit til Microsoft Office og Google docs,
    sum kann rætta mállæruvillur og stavivillur í ymsum málum.
gramdivvun-links-0 = Microsoft Office
gramdivvun-links-1 = Google Docs
islex = ISLEX
islex-description = Ein fleirmálslig netorðabók millum íslendskt og norðurlendsk mál (føroyskt, danskt, norskt bókmál, nýnorskt, svenskt og finskt).
islex-links-0 = ISLEX
